...
public class HeaderWriter {
  ...
  public int writeLocalFileHeader(ZipModel zipModel, LocalFileHeader localFileHeader,
    ...
    try {
      ...
      Raw.writeIntLittleEndian(intByte, 0, (int) localFileHeader.getSignature().getValue());
      ...
      //Compression Method
      Raw.writeShortLittleEndian(shortByte, 0, (short) localFileHeader.getCompressionMethod().getCode());
      copyByteArrayToArrayList(shortByte, byteArrayList);
      //File modified time
      int dateTime = localFileHeader.getLastModifiedTime();
      ...
      if (uncompressedSize + ZIP64_EXTRA_BUF >= ZIP_64_LIMIT) {
        Raw.writeLongLittleEndian(longByte, 0, ZIP_64_LIMIT);
      ...
      if (writingZip64Rec) {
        ...
        //Zip64 header
        Raw.writeShortLittleEndian(shortByte, 0, (short) HeaderSignature.ZIP64_EXTRA_FIELD_LENGTH.getValue());
        copyByteArrayToArrayList(shortByte, byteArrayList);
      ...
      if (localFileHeader.getAesExtraDataRecord() != null) {
        ...
        Raw.writeShortLittleEndian(shortByte, 0, (short) aesExtraDataRecord.getSignature().getValue());
        copyByteArrayToArrayList(shortByte, byteArrayList);
        ...
        Raw.writeShortLittleEndian(shortByte, 0, (short) aesExtraDataRecord.getCompressionMethod().getCode());
        copyByteArrayToArrayList(shortByte, byteArrayList);
      }
  ...
  public int writeExtendedLocalHeader(LocalFileHeader localFileHeader,
    ...
    Raw.writeIntLittleEndian(intByte, 0, (int) HeaderSignature.EXTRA_DATA_RECORD.getValue());
    copyByteArrayToArrayList(intByte, byteArrayList);
  ...
  public void finalizeZipFile(ZipModel zipModel, CountingOutputStream outputStream) throws ZipException {
    ...
    try {
      ...
      long offsetCentralDir = zipModel.getEndOfCentralDirRecord().getOffsetOfStartOfCentralDir();
      ...
      if (zipModel.isZip64Format()) {
        if (zipModel.getZip64EndOfCentralDirRecord() == null) {
          zipModel.setZip64EndOfCentralDirRecord(new Zip64EndOfCentralDirRecord());
        }
        if (zipModel.getZip64EndOfCentralDirLocator() == null) {
          zipModel.setZip64EndOfCentralDirLocator(new Zip64EndOfCentralDirLocator());
        }
  ...
  private void processHeaderData(ZipModel zipModel, OutputStream outputStream) throws ZipException {
    try {
      ...
      if (zipModel.isZip64Format()) {
        ...
        zipModel.getZip64EndOfCentralDirLocator().setNoOfDiskStartOfZip64EndOfCentralDirRec(currSplitFileCounter);
        zipModel.getZip64EndOfCentralDirLocator().setTotNumberOfDiscs(currSplitFileCounter + 1);
      }
      zipModel.getEndOfCentralDirRecord().setNoOfThisDisk(currSplitFileCounter);
      zipModel.getEndOfCentralDirRecord().setNoOfThisDiskStartOfCentralDir(currSplitFileCounter);
    } catch (IOException e) {
      throw new ZipException(e);
    }
  }
  ...
  private int writeFileHeader(ZipModel zipModel, FileHeader fileHeader,
    ...
    try {
      ...
      Raw.writeIntLittleEndian(intByte, 0, (int) fileHeader.getSignature().getValue());
      ...
      Raw.writeShortLittleEndian(shortByte, 0, (short) fileHeader.getCompressionMethod().getCode());
      copyByteArrayToArrayList(shortByte, headerBytesList);
      sizeOfFileHeader += 2;

      int lastModifiedTime = fileHeader.getLastModifiedTime();
      Raw.writeIntLittleEndian(intByte, 0, lastModifiedTime);
      copyByteArrayToArrayList(intByte, headerBytesList);
      sizeOfFileHeader += 4;
...
//			outputStream.write(byteArrayListToByteArray(headerBytesList));
  ...
  private void writeZip64EndOfCentralDirectoryRecord(ZipModel zipModel,
    ...
    try {
      ...
      Raw.writeIntLittleEndian(intByte, 0, (int) HeaderSignature.ZIP64_END_CENTRAL_DIRECTORY_RECORD.getValue());
      copyByteArrayToArrayList(intByte, headerBytesList);
      ...
      //number of this disk
      Raw.writeIntLittleEndian(intByte, 0, zipModel.getEndOfCentralDirRecord().getNoOfThisDisk());
      copyByteArrayToArrayList(intByte, headerBytesList);

      //number of the disk with start of central directory
      Raw.writeIntLittleEndian(intByte, 0, zipModel.getEndOfCentralDirRecord().getNoOfThisDiskStartOfCentralDir());
      copyByteArrayToArrayList(intByte, headerBytesList);
      ...
      if (zipModel.getCentralDirectory() == null ||
        ...
        if (zipModel.isSplitArchive()) {
          countNumberOfFileHeaderEntriesOnDisk(zipModel.getCentralDirectory().getFileHeaders(),
              zipModel.getEndOfCentralDirRecord().getNoOfThisDisk());
        } else {
  ...
  private void writeZip64EndOfCentralDirectoryLocator(ZipModel zipModel,
    ...
    try {
      ...
      Raw.writeIntLittleEndian(intByte, 0, (int) HeaderSignature.ZIP64_END_CENTRAL_DIRECTORY_LOCATOR.getValue());
      copyByteArrayToArrayList(intByte, headerBytesList);

      //number of the disk with the start of the zip64 end of central directory
      Raw.writeIntLittleEndian(intByte, 0, zipModel.getZip64EndOfCentralDirLocator().getNoOfDiskStartOfZip64EndOfCentralDirRec());
      copyByteArrayToArrayList(intByte, headerBytesList);

      //relative offset of the zip64 end of central directory record
      Raw.writeLongLittleEndian(longByte, 0, zipModel.getZip64EndOfCentralDirLocator().getOffsetZip64EndOfCentralDirRec());
      copyByteArrayToArrayList(longByte, headerBytesList);

      //total number of disks
      Raw.writeIntLittleEndian(intByte, 0, zipModel.getZip64EndOfCentralDirLocator().getTotNumberOfDiscs());
      copyByteArrayToArrayList(intByte, headerBytesList);
    } catch (ZipException zipException) {
      throw zipException;
    } catch (Exception e) {
  ...
  private void writeEndOfCentralDirectoryRecord(ZipModel zipModel,
    ...
    try {
      ...
      Raw.writeIntLittleEndian(intByte, 0, (int) zipModel.getEndOfCentralDirRecord().getSignature().getValue());
      copyByteArrayToArrayList(intByte, headrBytesList);

      //number of this disk
      Raw.writeShortLittleEndian(shortByte, 0, (short) (zipModel.getEndOfCentralDirRecord().getNoOfThisDisk()));
      copyByteArrayToArrayList(shortByte, headrBytesList);

      //number of the disk with start of central directory
      Raw.writeShortLittleEndian(shortByte, 0, (short) (zipModel.getEndOfCentralDirRecord().getNoOfThisDiskStartOfCentralDir()));
      copyByteArrayToArrayList(shortByte, headrBytesList);
...
//				Raw.writeIntLittleEndian(intByte, 0, (int)offsetCentralDir);
      ...
      int commentLength = 0;
      if (zipModel.getEndOfCentralDirRecord().getComment() != null) {
        commentLength = zipModel.getEndOfCentralDirRecord().getCommentLength();
      }
      ...
      //Comment
      if (commentLength > 0) {
        copyByteArrayToArrayList(zipModel.getEndOfCentralDirRecord().getCommentBytes(), headrBytesList);
      }

...
class ZipEntryInputStream extends InputStream {
  ...
  private String getNextSplitFileName() throws IOException {
    if (currentSplitFileCounter == zipModel.getEndOfCentralDirRecord().getNoOfThisDisk()) {
      return zipModel.getZipFile().getCanonicalPath();
    }

    String currZipFile = zipModel.getZipFile().getCanonicalPath();
